...
def tab2ply(tabGDB, tabset, resGDB, conAttr, fieldX, fieldY, fieldH, spatialReferencePRJ, tabNameMapping):
    ...
    for each in tabset:
        ...
        if count:
            ...
            print(resGDB)
            if type(resGDB) not in ("str", "unicode"):
                pass
            else:
                pass
...
def tab2plg(tabGDB, tabset, resGDB, fieldSerialNumber, spatialReferencePRJ):
    ...
    for each in tabset:
        ...
        if count:
            ...
            pntLayer = arcpy.MakeXYEventLayer_management(pntTabLayer, "X", "Y", "pntLayer", SR)
...
conAttr = arcpy.GetParameterAsText(8)
